...
class QuoteTotalsUnitTest {
    ...
    fun `non-terminating discount produces correct rounded result`() {
        ...
        val expected = BigDecimal("99.99")
            .multiply(BigDecimal.ONE.subtract(BigDecimal("33.33").divide(BigDecimal(100), 10, RoundingMode.HALF_UP)))
            .setScale(2, RoundingMode.HALF_UP)
    ...
    fun `grand total equals subTotal minus discount plus tax plus adjustment`() {
        ...
        val subTotal = quoteLineTotal(unitPrice, quantity, lineDiscount)
            .setScale(2, RoundingMode.HALF_UP)
        // subTotal = 200.00

        val afterDiscount = subTotal.multiply(
    ...
    fun `grand total with non-terminating tax percent does not throw`() {
        ...
        assertDoesNotThrow {
            ...
            val taxAmount = afterDiscount.multiply(
    ...
    fun `zero tax and zero discount - grand total equals sub total`() {
        ...
        val subTotal = quoteLineTotal(unitPrice, quantity, BigDecimal.ZERO)
            .setScale(2, RoundingMode.HALF_UP)
        val afterDiscount = subTotal.multiply(
