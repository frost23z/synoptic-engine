package com.synopticengine.api.crm

object CrmPermissions {
    const val TAGS = "tags"
    const val TAGS_VIEW = "tags.view"
    const val TAGS_CREATE = "tags.create"
    const val TAGS_EDIT = "tags.edit"
    const val TAGS_DELETE = "tags.delete"

    const val LEADS = "leads"
    const val LEADS_VIEW = "leads.view"
    const val LEADS_CREATE = "leads.create"
    const val LEADS_EDIT = "leads.edit"
    const val LEADS_DELETE = "leads.delete"

    const val CONTACTS = "contacts"
    const val CONTACTS_VIEW = "contacts.view"
    const val CONTACTS_CREATE = "contacts.create"
    const val CONTACTS_EDIT = "contacts.edit"
    const val CONTACTS_DELETE = "contacts.delete"
    const val CONTACTS_PERSONS = "contacts.persons"
    const val CONTACTS_PERSONS_VIEW = "contacts.persons.view"
    const val CONTACTS_PERSONS_CREATE = "contacts.persons.create"
    const val CONTACTS_PERSONS_EDIT = "contacts.persons.edit"
    const val CONTACTS_PERSONS_DELETE = "contacts.persons.delete"
    const val CONTACTS_ORGANIZATIONS = "contacts.organizations"
    const val CONTACTS_ORGANIZATIONS_VIEW = "contacts.organizations.view"
    const val CONTACTS_ORGANIZATIONS_CREATE = "contacts.organizations.create"
    const val CONTACTS_ORGANIZATIONS_EDIT = "contacts.organizations.edit"
    const val CONTACTS_ORGANIZATIONS_DELETE = "contacts.organizations.delete"

    const val ACTIVITIES = "activities"
    const val ACTIVITIES_VIEW = "activities.view"
    const val ACTIVITIES_CREATE = "activities.create"
    const val ACTIVITIES_EDIT = "activities.edit"
    const val ACTIVITIES_DELETE = "activities.delete"

    const val QUOTES = "quotes"
    const val QUOTES_VIEW = "quotes.view"
    const val QUOTES_CREATE = "quotes.create"
    const val QUOTES_EDIT = "quotes.edit"
    const val QUOTES_DELETE = "quotes.delete"

    const val MAIL = "mail"
    const val MAIL_VIEW = "mail.view"
    const val MAIL_EDIT = "mail.edit"
    const val MAIL_INBOX = "mail.inbox"
    const val MAIL_SENT = "mail.sent"
    const val MAIL_DRAFTS = "mail.drafts"
    const val MAIL_TRASH = "mail.trash"
    const val MAIL_SPAM = "mail.spam"
    const val MAIL_OUTBOX = "mail.outbox"

    const val PIPELINES = "pipelines"
    const val PIPELINES_VIEW = "pipelines.view"
    const val PIPELINES_CREATE = "pipelines.create"
    const val PIPELINES_EDIT = "pipelines.edit"

    const val REPORTS = "reports"
    const val REPORTS_VIEW = "reports.view"

    /** Saved datagrid column filters — personal-scope settings for each user. */
    const val DATAGRID_FILTERS = "datagrid-filters"
    const val DATAGRID_FILTERS_VIEW = "datagrid-filters.view"
    const val DATAGRID_FILTERS_EDIT = "datagrid-filters.edit"
}
