package com.synopticengine.api.settings

object SettingsPermissions {
    const val SETTINGS = "settings"
    const val SETTINGS_VIEW = "settings.view"
    const val SETTINGS_EDIT = "settings.edit"

    const val ATTRIBUTES = "attributes"
    const val ATTRIBUTES_VIEW = "attributes.view"
    const val ATTRIBUTES_EDIT = "attributes.edit"
    const val ATTRIBUTES_DELETE = "attributes.delete"

    const val AUTOMATIONS = "automations"
    const val AUTOMATIONS_VIEW = "automations.view"
    const val AUTOMATIONS_EDIT = "automations.edit"
    const val AUTOMATIONS_DELETE = "automations.delete"

    const val MARKETING = "marketing"
    const val MARKETING_VIEW = "marketing.view"
    const val MARKETING_EDIT = "marketing.edit"
    const val MARKETING_DELETE = "marketing.delete"

    const val IMPORTS = "imports"
    const val IMPORTS_VIEW = "imports.view"
    const val IMPORTS_EDIT = "imports.edit"
}
